    ...
    put: (data, callback) => {
        ...
        if(phone){
            ...
            if(firstName || lastName || password){
                ...
                callback(400, {"Error": "At least one field is required to update"})
            }
        }else{
                    ...
                    _data.read('users', phone, function(err, data){
                        if(!err && data){
                            _data.delete("users", phone, (err) => {
                                if(!err){
                                    // TODO: Delete the associated checks with user
                                    let userChecks = data.checks || [];
                                    let checksToDelete = userChecks.length;
                                    let errorAtDeletion = false;
                                    if(checksToDelete > 0){
                                        let checkDeleted = 0;
                                        userChecks.forEach(checkId => {
                                            _data.delete("checks", checkId, function(err){
                                                if(err){
                                                    errorAtDeletion = true;
                                                }
                                                checkDeleted++;
                                                if(checkDeleted === checksToDelete){
                                                    if(!errorAtDeletion){
                                                        callback(200, {"Message": "User Deleted with all checks successfully"})
                                                    }else{
                                                        callback(500, {"Error": "Error encountered while deleting checks associated with user"})
                                                    }
                                                }
